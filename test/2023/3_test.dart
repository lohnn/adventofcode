...
typedef Part = ({int x, int y, String partNumber});
typedef Gear = ({int x, int y});

extension on Part {
  int get width => partNumber.length;

  int get leftSide => x;

  int get rightSide => x + width - 1;
}

void main() {
  runDay(
    3,
    input1: ((testInput, 4361), (input, 535078)),
    input2: ((testInput, 467835), (input, 75312571)),
    part1: (lines) {
      final partNumbers = <Part>[
        for (final (row, line) in lines.indexed) ...extractParts(line, row),
      ];

      final partsWithAdjacentSymbols = [
        for (final part in partNumbers)
          if (part case _ when hasAdjacentSymbol(part, lines))
            int.parse(part.partNumber),
      ];

      return partsWithAdjacentSymbols.sum;
    },
    part2: (lines) {
      final partNumbers = <Part>[
        for (final (row, line) in lines.indexed) ...extractParts(line, row),
      ...
      final gears = <Gear>[
        for (final (row, line) in lines.indexed)
          for (final match in starRegex.allMatches(line))
            (x: match.start, y: row),
      ];

      final starPartMatch = <Gear, List<Part>>{};

      for (final part in partNumbers) {
        for (final gear in gears) {
          if (isAdjacent(part, gear)) {
            starPartMatch.update(
              gear,
      ...
      final exactMatches = starPartMatch.values
          .where((element) => element.length == 2)
          .map((e) => int.parse(e[0].partNumber) * int.parse(e[1].partNumber));

      return exactMatches.sum;
    },
  );
}

bool isAdjacent(Part part, Gear gear) {
  // Outside of horizontal bounds
  if (gear.x < part.leftSide - 1) return false;
  if (gear.x > part.rightSide + 1) return false;

  // Outside of vertical bounds
  if (gear.y < part.y - 1) return false;
  if (gear.y > part.y + 1) return false;

  return true;
}

final symbolRegex = RegExp(r'[0-9]|[^.]');
...
bool hasAdjacentSymbol(
  ...
  // First check above
  if (part.y != 0) {
    final lineAbove = lines[part.y - 1];
    if (symbolRegex.hasMatch(
      lineAbove.substring(
        max(0, part.x - 1),
        min(lineAbove.length - 1, part.x + part.width + 1),
      ),
    )) {
      return true;
    }
  ...
  // Check one left
  final sameLine = lines[part.y];
  if (part.x > 0 && sameLine.codeUnits[part.x - 1] != 46) return true;
  // Check one right
  if (part.x + part.width < sameLine.length &&
      sameLine.codeUnits[part.x + part.width] != 46) {
    return true;
  }

  // Check below
  if (part.y != lines.length - 1) {
    final lineBelow = lines[part.y + 1];
    if (symbolRegex.hasMatch(
      lineBelow.substring(
        max(0, part.x - 1),
        min(lineBelow.length - 1, part.x + part.width + 1),
      ),
    )) {
      return true;
    }
