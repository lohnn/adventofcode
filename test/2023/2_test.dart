...
import '../advent_setup.dart';

void main() {
  runDay(
    2,
    input1: ((testInput, 8), (input, 2101)),
    input2: ((testInput, 2286), (input, 58269)),
    part1: (lines) {
      final possibleGameIds = [
        for (final line in lines)
          if (extractPartOne(line) case final id?) id,
      ];
    ...
    part2: (lines) {
      final possibleCubes = [
        for (final line in lines)
          if (extractPartTwo(line) case final game)
            game.red * game.green * game.blue,
      ];
      return possibleCubes.sum;
    },
  );
}

int? extractPartOne(String line) {
  final split = line.split(':');
  final id = int.parse(split.first.substring(5));

  for (final set in split[1].split(';')) {
    for (final color in set.split(',')) {
      switch (color.split(' ')) {
        case [_, final amount, 'green'] when int.parse(amount) > green:
          return null;
        case [_, final amount, 'blue'] when int.parse(amount) > blue:
          return null;
        case [_, final amount, 'red'] when int.parse(amount) > red:
          return null;
      }
    }
  }
...
({int red, int green, int blue}) extractPartTwo(String line) {
  final split = line.split(':');
  ...
  for (final set in split[1].split(';')) {
    for (final color in set.split(',')) {
      switch (color.split(' ')) {
        case [_, final amount, 'green']:
          green = max(green, int.parse(amount));
        case [_, final amount, 'blue']:
          blue = max(blue, int.parse(amount));
        case [_, final amount, 'red']:
          red = max(red, int.parse(amount));
      }
    }
  }
